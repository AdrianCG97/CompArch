...
//LAB 5
int GetGATE_PTE_A(int *x)    { return(x[GATE_PTE_A]); }
...
int GetT(int *x)             { return(x[T]); }
int GetFT(int *x)            { return(x[FT]); }
...
int PTE;  /* Temporary Page Table Entry register */
int SAT;  /* Holds next state after translation */
...
int getG_PA(){
    return ((CURRENT_LATCHES.VA & 0x01FF) + (CURRENT_LATCHES.PTE & 0x3E00));
}
int getG_PTE(){
    return CURRENT_LATCHES.PTE;
}
...
void PTE_latch(){
    if(GetLD_PTE(mic)){
        int mux = GetPTEMUX(mic);
        if(mux == 0){
            NEXT_LATCHES.PTE = BUS;
        }
        else{
            int ns = CURRENT_LATCHES.SAT;
            int isWrite = 0x0000;
            if(ns == 16 || ns == 17) isWrite = 0x0002;
            NEXT_LATCHES.PTE =  (CURRENT_LATCHES.PTE | 0x0001 | isWrite);
            //printf("Latching PTE = %x or x0001 or %x \n",CURRENT_LATCHES.PTE,isWrite);          //<<---------------------------------------------------------------------------------
        }
    }
    else{
        NEXT_LATCHES.PTE = CURRENT_LATCHES.PTE;
    }
...
uint16_t Gmar,Gpc,Galu,Gshf,Gmdr;
uint16_t Gr8,Gusp,Gssp,Gpsr,Gvec; //values in gates 
uint16_t Gptea,Gpa,Gpte;
...
void drive_bus() {
   ...
   */       
    ...
    else if(GetGATE_PTE_A(mic)){
        BUS = Gptea;
    }
    else if(GetGATE_PA(mic)){
        BUS = Gpa;
    }
    else if(GetGATE_PTE(mic)){
        BUS = Gpte;
    }
    else{
        BUS = 0;
...
void Ints_and_Excp(){
    ...
    int mode = (CURRENT_LATCHES.PSR & 0x8000) >> 15;       //One is User Mode
    int isEI = 0;
    int vecTemp = 0;

    //Check for Unaligned exceptions
    if(ce  == 1){
        int sat = CURRENT_LATCHES.SAT;
        if((CURRENT_LATCHES.MAR & 0x0001) & (sat == 25 || sat == 16)){
            isEI = 1;
            vecTemp = 3;
        }
    }
    //Check for Protection exceptions or page fault exceptions
    if(ce  ==  2){
        //Protection
        int nState = CURRENT_LATCHES.SAT;
        if(mode && (CURRENT_LATCHES.PTE & 0x0008) && (nState != 28)){
            isEI = 1;
            vecTemp = 4;
        }
        //Page Fault
        else if(!(CURRENT_LATCHES.PTE & 0x0004)){
    ...
    if(isEI && !t){ //Exception
        ...
        //printf("Starting exception\n");

        for(int i = 0; i < CONTROL_STORE_BITS; i++){
            NEXT_LATCHES.MICROINSTRUCTION[i] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][i];
        }
    }
    else if(!isEI && t){    //Go to Address Translation
        NEXT_LATCHES.SAT = NEXT_LATCHES.STATE_NUMBER;
    ...
    else if(isEI && t){ //Finish Address Translation Routine
        NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.SAT;

        for(int i = 0; i < CONTROL_STORE_BITS; i++){
            NEXT_LATCHES.MICROINSTRUCTION[i] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][i];
        }
    }
    //else if(INTflag && (NEXT_LATCHES.STATE_NUMBER == 18)){
    else if(INTflag){
        INTflag = 0;
        NEXT_LATCHES = CURRENT_LATCHES;
        NEXT_LATCHES.STATE_NUMBER = 43;
    ...
    //    printf("Done with interrupt/Exception \n");
    //}
}
